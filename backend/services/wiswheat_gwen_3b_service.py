...
class WisWheat_Gwen_3BService:
    def __init__(self, device_map: str = "cuda"):
        self.model_name = "WisWheat/WisWheat_Qwen-3B"
        ...
        logger.info(f"WisWheat-Gwen-3B using device: {self.device}")
    ...
    def load_model(self) -> bool:
        if self.is_loaded:
            logger.info("WisWheat-Gwen-3B model already loaded")
            return True
            
        try:
            logger.info(f"Loading WisWheat-Gwen-3B model: {self.model_name}")
            ...
            self.is_loaded = True
            logger.info(f"WisWheat-Gwen-3B model loaded successfully with memory optimizations.")
            return True
            
        except Exception as e:
            logger.error(f"Failed to load WisWheat-Gwen-3B model: {str(e)}")
            self._cleanup()
            return False
    ...
    def _cleanup(self):
        ...
        logger.info("WisWheat-Gwen-3B model cleanup completed")
    ...
    def generate_response(self, 
        ...
        try:
            ...
            logger.error(f"Error during WisWheat-Gwen-3B response generation: {str(e)}")
    ...
    def generate_response_stream(self, 
        ...
        try:
            ...
            logger.error(f"Error during WisWheat-Gwen-3B streaming generation: {str(e)}")
            # Clear cache on error
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            raise
